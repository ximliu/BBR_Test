...
dump_flow()
{
	...
	cmd="$cmd printf \"total sessions: $HL_START%d$HL_END, \" $NumOfFlows;"
	...
	#echo
	cmd="$cmd printf \"\${showLan:+wan }in: $HL_START%.2F$HL_END kbit/s\t\${showLan:+wan }out: $HL_START%.2F$HL_END kbit/s \" $wanInRate $wanOutRate;"
	[ -n "$showLan" ] && {
		cmd="$cmd printf \"lan out: $HL_START%.2F$HL_END kbit/s\tlan in:  $HL_START%.2F$HL_END kbit/s \" $lanOutRate $lanInRate;"
...
dump_ipv4_flow()
{
	cmd="$cmd printf \"ipv4 sessions: $HL_START%d$HL_END, \" $V4NumOfFlows;"
	cmd="$cmd printf \"tcp sessions: $HL_START%d$HL_END, \" $V4NumOfTcpFlows;"
	cmd="$cmd printf \"accelerated tcp sessions: $HL_START%d$HL_END, \" $V4NumOfAccFlows;"
	cmd="$cmd printf \"active tcp sessions: $HL_START%d$HL_END, \" $V4NumOfActFlows;"
	[ $NfBypass -gt 0 ] && cmd="$cmd printf \"Short-RTT bypassed packets: $HL_START%d$HL_END\n\" $NfBypass;"
	#echo
	cmd="$cmd printf \"\${showLan:+wan }in: $HL_START%.2F$HL_END kbit/s\t\${showLan:+wan }out: $HL_START%.2F$HL_END kbit/s \" $v4wanInRate $v4wanOutRate;"
	[ -n "$showLan" ] && {
		cmd="$cmd printf \"lan out: $HL_START%.2F$HL_END kbit/s\tlan in:  $HL_START%.2F$HL_END kbit/s \" $v4lanOutRate $v4lanInRate;"
		cmd="$cmd printf \"retransmission ratio: $HL_START%.1F %%$HL_END\" $v4outRatio;"
	}
	cmd="$cmd printf \"\n\";"
}
...
dump_ipv6_flow()
{
	cmd="$cmd printf \"ipv6 sessions: $HL_START%d$HL_END, \" $V6NumOfFlows;"
	cmd="$cmd printf \"tcp sessions: $HL_START%d$HL_END, \" $V6NumOfTcpFlows;"
	cmd="$cmd printf \"accelerated tcp sessions: $HL_START%d$HL_END, \" $V6NumOfAccFlows;"
	cmd="$cmd printf \"active tcp sessions: $HL_START%d$HL_END, \" $V6NumOfActFlows;"
	#echo
	cmd="$cmd printf \"\${showLan:+wan }in: $HL_START%.2F$HL_END kbit/s\t\${showLan:+wan }out: $HL_START%.2F$HL_END kbit/s \" $v6wanInRate $v6wanOutRate;"
	[ -n "$showLan" ] && {
		cmd="$cmd printf \"lan out: $HL_START%.2F$HL_END kbit/s\tlan in:  $HL_START%.2F$HL_END kbit/s \" $v6lanOutRate $v6lanInRate;"
		cmd="$cmd printf \"retransmission ratio: $HL_START%.1F %%$HL_END\" $v6outRatio;"
	}
	cmd="$cmd printf \"\n\";"
}
...
dump_total_flow()
{
	...
	cmd="$cmd printf \"total sessions: $HL_START%d$HL_END, \" $NumOfFlowsTotal;"
	...
	#echo
	cmd="$cmd printf \"\${showLan:+wan }in :  $HL_START%.2F$HL_END kbit/s\t\${showLan:+wan }out: $HL_START%.2F$HL_END kbit/s\" $wanInRateTotal $wanOutRateTotal;"
	[ -n "$showLan" ] && {
		cmd="$cmd printf \" lan out: $HL_START%.2F$HL_END kbit/s\tlan in :  $HL_START%.2F$HL_END kbit/s\" $lanOutRateTotal $lanInRateTotal;"
...
{
	cmd="$cmd printf \"ipv4 sessions: $HL_START%d$HL_END, \" $V4NumOfFlowsTotal;"
	cmd="$cmd printf \"tcp sessions: $HL_START%d$HL_END, \" $V4NumOfTcpFlowsTotal;"
	cmd="$cmd printf \"accelerated tcp sessions: $HL_START%d$HL_END, \" $V4NumOfAccFlowsTotal;"
	cmd="$cmd printf \"active tcp sessions: $HL_START%d$HL_END, \" $V4NumOfActFlowsTotal;"
	cmd="$cmd printf \"Short-RTT bypassed packets: $HL_START%d$HL_END\n\" $NfBypassTotal;"
	#echo
	cmd="$cmd printf \"\${showLan:+wan }in :  $HL_START%.2F$HL_END kbit/s\t\${showLan:+wan }out: $HL_START%.2F$HL_END kbit/s\" $v4wanInRateTotal $v4wanOutRateTotal;"
	[ -n "$showLan" ] && {
		cmd="$cmd printf \" lan out: $HL_START%.2F$HL_END kbit/s\tlan in :  $HL_START%.2F$HL_END kbit/s\" $v4lanOutRateTotal $v4lanInRateTotal;"
...
{
	cmd="$cmd printf \"ipv6 sessions: $HL_START%d$HL_END, \" $V6NumOfFlowsTotal;"
	cmd="$cmd printf \"tcp sessions: $HL_START%d$HL_END, \" $V6NumOfTcpFlowsTotal;"
	cmd="$cmd printf \"accelerated tcp sessions: $HL_START%d$HL_END, \" $V6NumOfAccFlowsTotal;"
	cmd="$cmd printf \"active tcp sessions: $HL_START%d$HL_END, \" $V6NumOfActFlowsTotal;"
#	cmd="$cmd printf \"Short-RTT bypassed packets: $HL_START%d$HL_END\n\" $NfBypassTotal;"
	#echo
	cmd="$cmd printf \"\${showLan:+wan }in :  $HL_START%.2F$HL_END kbit/s\t\${showLan:+wan }out: $HL_START%.2F$HL_END kbit/s\" $v6wanInRateTotal $v6wanOutRateTotal;"
	[ -n "$showLan" ] && {
		cmd="$cmd printf \" lan out: $HL_START%.2F$HL_END kbit/s\tlan in :  $HL_START%.2F$HL_END kbit/s\" $v6lanOutRateTotal $v6lanInRateTotal;"
